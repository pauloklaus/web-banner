...
const iconPaths =
  sourceSvg.replace(/currentColor/g, '#4a9eff').match(/<g[\s\S]*<\/g>/)?.[0] ??
  sourceSvg

const ogLocales: Record<string, string> = {
  en: 'Scrolling text banner',
  pt: 'Banner de texto rolante',
  es: 'Banner de texto desplazable',
}
...
function buildOgImage(subtitle: string): string {
  ...
  <rect x="80" y="80" width="470" height="470" rx="48" fill="#242424"/>
  <svg x="155" y="155" width="320" height="320" viewBox="0 0 24 24">
    ${iconPaths}
  </svg>
  <text x="620" y="250" fill="#ffffff" font-family="${OG_FONT_FAMILY}" font-size="72" font-weight="700">WebBanner</text>
  <text x="620" y="340" fill="#aaaaaa" font-family="${OG_FONT_FAMILY}" font-size="36">${subtitle}</text>
  <text x="620" y="420" fill="#4a9eff" font-family="${OG_FONT_FAMILY}" font-size="28">banner.pauloklaus.com.br</text>
</svg>`
}
...
await writePng('favicon-light-32.png', buildFaviconBadge(32, '#1a1a1a'), 32)
await writePng('favicon-dark-32.png', buildFaviconBadge(32, '#ffffff'), 32)
await writePng('favicon-light-16.png', buildFaviconBadge(16, '#1a1a1a'), 16)
await writePng('favicon-dark-16.png', buildFaviconBadge(16, '#ffffff'), 16)
